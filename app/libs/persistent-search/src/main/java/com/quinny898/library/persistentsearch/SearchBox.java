...
import com.com.mcochin.custom.BackPressEditText;
...
public class SearchBox extends RelativeLayout {
	...
	private BackPressEditText search;
	...
	public SearchBox(final Context context, AttributeSet attrs, int defStyle) {
		...
		this.search = (BackPressEditText) findViewById(R.id.search);
		...
		search.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable s) {
				if (s.length() > 0) {
					...
					mic.setImageDrawable(ContextCompat.getDrawable(getContext(),
							R.drawable.ic_clear));
					updateResults();
				} else {
